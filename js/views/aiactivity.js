...
function _vStAiaDo(){
  ...
  const savingChip = AIA_SAVING ? '<span class="aia-meta saving">💾 저장 중...</span>' : '';
  ...
  return `<div class="back-btn" data-action="aia-back">← 활동 목록으로</div>
    ...
    <div class="aia-do-foot">
      ${updatedLabel}
      ${savingChip}
      <button class="btn-p btn-sm" data-action="aia-save" ${AIA_SAVING ? 'disabled' : ''}>💾 저장</button>
    </div>`;
}
...
function _vTcAiaStudentList(){
  ...
  const submittedCount = STUDENTS.filter(st => subs[st.number]?.updatedAt).length;
  ...
  <div class="asmt-stat-grid">
    ...
    <div class="stat-card"><div class="stat-num" style="color:var(--ok)">${submittedCount}</div><div class="stat-label">작성한 학생</div></div>
  </div>
  ${STUDENTS.length === 0
    ? emptyBox('👥', '먼저 학생을 등록하세요.')
    : `<div style="overflow-x:auto"><table class="tbl aia-tc-table">
        <thead><tr><th>학번</th><th>이름</th><th>작성률</th><th>마지막 저장</th><th></th></tr></thead>
        <tbody>${rows}</tbody>
      </table></div>`
  }`;
}
...
function _vTcAiaStudent(){
  ...
  const back = `<div class="aia-tcs-header">
    ...
    <div class="aia-tcs-info">
      ...
      ${sub?.updatedAt ? `<span class="chip chip-green">✓ ${fmtDt(sub.updatedAt)}</span>` : '<span class="chip">미작성</span>'}
    </div>
  </div>`;
