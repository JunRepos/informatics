# 🔄 세션 핸드오프 (2026-06-15 갱신) — ML 수행평가 문서 검토·개정 + 세특 강화

> 워크트리 **claude/angry-albattani-2694b8** / 배포 `git push origin <브랜치>:main` + curl 확인.
> 코드(앱) main = **35e34dc** (라이브 반영 완료). 문서 작업은 대부분 **구글드라이브 docx**(코드 아님).
> 환경: node 없음 · python(PYTHONUTF8=1 필수, 한글 콘솔 깨짐→파일 저장 후 Read) · docx는 스킬 unpack/pack 또는 python-docx · PDF 변환은 **PowerShell Word COM**(LibreOffice·pdftoppm 없음) · 프리뷰 JS 캐시 때문에 검증 시 새 포트.

## ⭐ 즉시 할 일 — 세특 강화 v3 (사용자 승인 대기 중이었음)
검색·검토 끝났고 **내용 확정**. v2 docx에 아래를 더해 **v3로 생성**만 하면 됨(메뉴형·15점·채점 불변, 추가분 전부 **비채점**). 근거 자료: `_setuk_ctx.md`(워크트리).

**(1) 활동지 v3** — `수행평가_활동지_학생용_v2.docx` 문항3 아래에 **비채점 "성찰 한 줄 카드"** 박스 추가:
- 머리말: "[성찰 한 줄 카드 · 점수에 반영되지 않아요] 정답 없는 칸. 한 줄이면 충분. 솔직히 쓴 내용은 선생님이 세특 기록에 참고."
- ① 머신러닝/AI에 새로 알게 됐거나 생각이 달라진 점? (예: 분류와 군집 차이를 처음 구분)
- ② 가장 헷갈린 부분과 "내가 어떻게" 판단했나? (예: 답이 숫자인지 종류인지로 따져봄)
- ③ 더 발전시킨다면 뭘 더 알아보거나 바꿔보고 싶나? (□특징추가 □다른모델 □데이터더 + 한 줄)
- ④ 오늘 AI 방식을 내 관심·일상에 쓴다면 어디에? (꼭 장래희망 아니어도)
- ⑤ 이 AI를 "믿으면 안 되는" 상황은? (예: 학습 데이터에 없던 새 손님이면 추천 틀림)
- (각 칸 빈줄 2줄. 학생 +5~7분. ①②=학업·문제해결, ③=후속, ④=진로(억지태깅 회피형), ⑤=윤리·편향)

**(2) 운영안 v3** — `수행평가_운영안_정답_교사용_v2.docx`에 **"세특 작성 가이드"** 섹션 + 샘플 문장 5종 추가:
- 가이드: 근거 3종(채점 산출물+성찰카드+5차시 AI 프로젝트 매니저 앱 기록 교차확인) / 원칙(탐구과정·구체근거·후속·자연스러운 진로수렴, 추상칭찬·억지직업태깅 금지) / 규칙(성찰카드 **그대로 옮기지 말 것**=교사 재구성, 안 쓴 내용 지어내기 금지=사실근거, 글자수 유의).
- 샘플 5종(역량 먼저→진로 자연수렴, 각 2~3문장): 분류/의료·회귀/환경·군집/마케팅·분류/교육·회귀/스포츠. (전문 문구는 이 세션 마지막 응답 참조 또는 _setuk_ctx 원칙으로 재작성.)
- **루브릭은 변경 없음**(v2 그대로).

→ 생성 방식: v2 docx를 스킬로 unpack→XML 편집→pack(새 파일 `_v3.docx`), 또는 python-docx로 재생성. **원본·v2 보존, v3 새 파일로.**

## ✅ 이번 세션 완료한 것
1. **수행평가 문서 검토(워크플로 123에이전트)**: 정답키 8상황32과제 **무오류** 확인. must-fix 4건(①척도 하한=백지6점 충돌→백지경계 정의 ②감점 하한·중복 규정 ③만점예시 군집·사진 추가 ④'~가능성/위험' 경계 일괄화), 토론쟁점 A~D(메뉴소거법·지도비지도 직접평가·신경망 인정·나의문제). → **전부 v2에 반영함**.
2. **v2 docx 3종 생성**(드라이브, python-docx): `수행평가_활동지_학생용_v2 / _채점루브릭_교사용_v2 / _운영안_정답_교사용_v2`. 원본(_4문항)·`백업_2026-06-11/` 보존. PDF QA 렌더 확인.
3. **세특 강화 검토**: 웹검색(세특 작성원리·기재규칙·진로강박금지)+워크플로(중단, 핵심 복구). → 위 v3 내용 확정.
4. **공지(A안)**: `수행평가_공지_반별게시.pdf` 변환 완료(드라이브). 공지 본문 텍스트 제공함. (드라이브에 `_간결판.docx`도 있음 — 사용자가 만든 듯.) **게시는 교사 직접**(미게시 여부 불명).
5. (이전 배포분) 앱: AI프로젝트매니저 v3 유형가설+의뢰서+길잡이, ML수행평가 디지털 활동지(개방형) — main 배포 완료. 메모리 [ml_problem_solving][ml_assessment] 참조.

## 📌 펜딩 / 다음 후보
- **개방형 종이 전환은 차후**(이번 차수는 메뉴형 v2/v3로). 사용자가 "개방형은 일단 차후" 명시.
- 공지 게시 확인(교사). 시험: 정보 2-A 6/16(화), 2-B 6/17(수), 정보수업 50분.
- (선택) v3 만든 뒤 PDF 렌더 QA + 서브에이전트 시각검수.

## 🗂️ 파일 위치
- 수행평가 docx: `D:\Google drive\신동고등학교\01_교과_정보\인공지능\` (v2 3종 + 공지 + 원본 + 백업_2026-06-11/)
- 수업 PPT v2: `C:\Users\PC\Desktop\인공지능 5차시 - 기계학습 구현 과정과 문제 해결 v2.pptx`(20장)
- 세특 근거: 워크트리 `_setuk_ctx.md`
- 앱 데이터: `js/ml-assessment-data.js`(8상황·dataItems), `js/views/mlassess.js`, `js/events/mlassess.js`
